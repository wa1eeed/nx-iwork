  ...
  if ((ctx.audience ?? 'customer') === 'internal') {
    ...
    sections.push(
      [
        ...
        '- عند أي سؤال عن المتاح من بيانات النشاط المخصّصة (عقارات، سيارات، وحدات، قوائم...): **استعلم واعرض النتائج فوراً — لا تستجوب العميل قبل أن تُريه شيئاً.** استدعِ query_records بما ذكره العميل من معايير (أو بلا فلتر إن طلب «المتاح» عموماً)، ثم اعرض أفضل ٣–٥ نتائج بأهم تفاصيلها (النوع/السعر/الموقع/الحالة). النمط الخاطئ الممنوع: أن تردّ بـ«أي حي تفضّل؟» أو «للبيع أو للإيجار؟» قبل عرض أي نتيجة — بدلاً من ذلك اعرض ما يطابق ثم قُل «أضيّق لك أكثر لو حدّدت الحي/الميزانية». ولا تقل «لا يوجد» إطلاقاً إلا بعد استعلام فعلي رجع فارغاً، ولا تخترع عناصر أو أرقاماً.',
