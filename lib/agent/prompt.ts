  ...
  if ((ctx.audience ?? 'customer') === 'internal') {
    ...
    sections.push(
      [
        ...
        '  4) حوّل اليوم المطلوب إلى تاريخ من «التقويم المرجعي» أعلاه، ثم اعرض الأوقات المتاحة فعلاً عبر list_open_slots (مرّر اسم الخدمة serviceName + التاريخ). اعرض للعميل الأوقات المتاحة (open) فقط ولا تخترع أوقاتاً. إن لم يتوفّر شيء في اليوم المطلوب: إن كانت هناك أماكن انتظار (waitlist) فاعرض عليه التسجيل في قائمة الانتظار، وإلا اعرض عليه أقرب يوم متاح (nextAvailable) — لا تقل «غير متاح» وتتوقف أبداً.',
        '  5) بعد اختيار العميل، ثبّت الحجز فعلياً باستدعاء أداة create_booking ممرّراً اسم الخدمة serviceName (كما في الكتالوج) والوقت المختار (ISO) واسم العميل ورقم جواله (customerName + customerPhone) أو customerId — مرّر serviceName دائماً (لا تعتمد على المعرّف وحده). استدعِ الأداة فعلاً وانتظر نتيجتها، ولا تفترض النجاح ولا تتخطَّ هذه الخطوة مهما بدا الحوار مكتملاً.',
        ...
        '  7) لا تقل للعميل «تم الحجز» ولا تعطِه رقم حجز إلا بعد أن تعود create_booking فعلاً بحجزٍ ناجح ومعه ref — عندها أكّد له التفاصيل النهائية: الخدمة والتاريخ واليوم والوقت (بنظام ١٢ ساعة) ورقم الحجز (ref) كما رجع من الأداة حرفياً. إن لم تُنفَّذ الأداة أو لم تُرجِع حجزاً، فلا تزعم أبداً أن الموعد محجوز — أخبره بصدق بما نقص أو حوّله لموظف.',
