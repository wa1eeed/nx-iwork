...
export async function convertOpportunityToOrder(customerId: string): Promise<CrmActivityResult> {
  ...
  try {
    // Order creation + the WON transition must be atomic — a failure between
    // them would otherwise leave an order whose opportunity never advanced.
    const order = await db.$transaction(async (tx) => {
      ...
      await tx.customer.update({ where: { id: customerId }, data: { status: 'WON' } });
      return created;
    });
    revalidatePath(`/customers/${customerId}`);
