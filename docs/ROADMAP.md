...
> 📌 **ملاحظة:** هذا هو **مخطط الـ Sprints الأصلي (تاريخي/مرجعي)**. للمهام
> القادمة الحيّة اعتمد [`TODO.md`](./TODO.md)، وللمُنجز [`../CHANGELOG.md`](../CHANGELOG.md).
...
## ✅ الحالة الحالية (مُنجز — يتجاوز خطة الـ Sprints الأصلية)

النواة الأساسية **مكتملة ومنشورة** على [bznss.one](https://bznss.one/). البناء جرى عبر
طبقات محايدة معزولة (تطوير أي جزء لا يلمس الباقي) لا حسب ترتيب الـ Sprints أدناه:

- ✅ **AI محايد** (Gemini افتراضي + Claude) BYOK لكل شركة — `lib/ai/`
- ✅ **محادثة الوكلاء + استدعاء الأدوات** (catalog, CRM, tasks, save_memory)
- ✅ **نموذج بيانات مرن**: CRM (`Customer`) + `customFields` + `Task` موحّد (migration `20260619130000`)
- ✅ **إدارة الأقسام والوكلاء** + بناء الشخصية
- ✅ **محرّك تنفيذ المهام** (دورة حياة + محاولات + Timeline)
- ✅ **الجدولة التلقائية** (worker مستقل `npm run scheduler`)
- ✅ **الذاكرة الدلالية** (pgvector + embeddings مع fallback)
- ✅ **تخزين R2** + كتالوج المنتجات · **إشعارات** (Resend + Twilio)
- ⬜ **التالي:** اللاندنغ بيج العامة `app/[slug]` + ودجت الوكيل المدمج
- ⬜ **لاحقاً:** Sentry (مراقبة) · Tap.company (اشتراكات) · API عام v1 · 3 بيئات (dev/staging/prod)

> خطة الـ Sprints أدناه **مرجع تاريخي** للتخطيط الأولي. الحالة المعتمدة: `docs/PROJECT.md §10` + `CHANGELOG.md`.
